this.Screen = class Screen {
  ...
  initResizeObserver() {
    if (typeof ResizeObserver !== 'undefined') {
      ...
      const parent = this.runtime.container || this.canvas.parentElement;
      if (parent) {
  ...
  checkMouseCursor() {
    const CURSOR_HIDE_TIMEOUT_MS = 4000;
    if (Date.now() > this.last_mouse_move + CURSOR_HIDE_TIMEOUT_MS && this.cursor_visibility === "auto") {
      if (this.cursor !== "none") {
        this.cursor = "none";
  ...
  initContext() {
    ...
    this.coordinate_scale = Math.min(this.canvas.width / 200, this.canvas.height / 200);
    this.context.scale(this.coordinate_scale, this.coordinate_scale);
    this.width = this.canvas.width / this.coordinate_scale;
    this.height = this.canvas.height / this.coordinate_scale;
  ...
  updateInterface() {
    ...
    this.interface.aspect = this.width / this.height;
    return this.interface.orientation = this.runtime.orientation;
  }
  ...
  drawSprite(sprite, x, y, w, h) {
    var canvas;
    var spriteName = typeof sprite === 'string' ? sprite : '(unknown)';
  ...
  resize() {
    if (!this.runtime.container && !this.canvas.parentElement) {
      return;
    }
    var backingStoreRatio, ch, cw, devicePixelRatio, h, min, parent, r, ratio, w;
    if (!this.runtime.aspect) {
      this.runtime.aspect = "free";
    }

    parent = this.runtime.container || this.canvas.parentElement;
    if (parent) {
      cw = parent.clientWidth;
      ch = parent.clientHeight;
      if (this.runtime.container) {
        let style = window.getComputedStyle(parent);
        cw -= parseFloat(style.paddingLeft) + parseFloat(style.paddingRight);
        ch -= parseFloat(style.paddingTop) + parseFloat(style.paddingBottom);
      }
    } else {
    ...
    if (this.runtime.aspect === "free") {
      devicePixelRatio = window.devicePixelRatio || 1;
      this.width = cw * devicePixelRatio;
      this.height = ch * devicePixelRatio;
    ...
    this.canvas.style["margin-top"] = (this.runtime.container) ? "0px" : Math.round((ch - h) / 2) + "px";
    this.canvas.style.width = Math.round(w) + "px";
    this.canvas.style.height = Math.round(h) + "px";
    devicePixelRatio = window.devicePixelRatio || 1;
    backingStoreRatio = this.context.webkitBackingStorePixelRatio || this.context.mozBackingStorePixelRatio || this.context.msBackingStorePixelRatio || this.context.oBackingStorePixelRatio || this.context.backingStorePixelRatio || 1;
    this.pixel_ratio = devicePixelRatio / backingStoreRatio * Math.max(1, Math.min(2, this.supersampling));
    this.width = w * this.pixel_ratio;
    this.height = h * this.pixel_ratio;
  ...
  startControl(element) {
    ...
    this.listeners = {
      ...
      touchend: (event) => this.touchRelease(event),
      touchcancel: (event) => this.touchRelease(event),
      mousedown: (event) => this.mouseDown(event),
      mousemove: (event) => this.mouseMove(event),
      mouseup: (event) => this.mouseUp(event),
      mousewheel: (event) => this.mouseWheel(event),
      DOMMouseScroll: (event) => this.mouseWheel(event),
    };
    ...
    devicePixelRatio = window.devicePixelRatio || 1;
    backingStoreRatio = this.context.webkitBackingStorePixelRatio || this.context.mozBackingStorePixelRatio || this.context.msBackingStorePixelRatio || this.context.oBackingStorePixelRatio || this.context.backingStorePixelRatio || 1;
    return this.pixel_ratio = devicePixelRatio / backingStoreRatio;
  }
  ...
  touchRelease(event) {
    ...
    for (i = j = 0, ref = event.changedTouches.length - 1; j <= ref; i = j += 1) {
      t = event.changedTouches[i];
      x = (t.clientX - this.canvas.offsetLeft) * this.pixel_ratio;
      y = (t.clientY - this.canvas.offsetTop) * this.pixel_ratio;
