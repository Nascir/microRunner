...
this.Runtime = class Runtime {
  constructor(url1, sources, resources, listener, canvas, options = {}) {
    ...
    this.orientation = options.orientation || window.orientation || "any";
    this.aspect = options.aspect || window.aspect || "free";
  ...
  start() {
    ...
    for (j = 0, len1 = ref.length; j < len1; j++) {
      i = ref[j];
      s = LoadSprite("/api/sprite/" + this.projectName + "/" + i.file + "?v=" + i.version, i.properties, () => {
        this.updateMaps();
    ...
    if (Array.isArray(this.resources.maps)) {
      ...
      for (k = 0, len2 = ref1.length; k < len2; k++) {
        ...
        this.maps[name] = LoadMap("/api/map/" + this.projectName + "/" + m.file + "?v=" + m.version, () => {
          this.maps[name].sprites = this.sprites;
          return this.checkStartReady();
        });
    ...
    for (l = 0, len3 = ref3.length; l < len3; l++) {
      s = ref3[l];
      name = s.file.split(".")[0];
      s = new Sound(this.audio, "/api/sound/" + this.projectName + "/" + s.file + "?v=" + s.version);
      s.name = name;
      this.sounds[name] = s;
    }
    ...
    for (n = 0, len4 = ref4.length; n < len4; n++) {
      m = ref4[n];
      name = m.file.split(".")[0];
      m = new Music(this.audio, "/api/music/" + this.projectName + "/" + m.file + "?v=" + m.version);
      m.name = name;
      this.music[name] = m;
    }
    ...
    // If no resources to load, check if ready to start
    if (this.resources.images.length === 0 &&
      (!Array.isArray(this.resources.maps) || this.resources.maps.length === 0) &&
      this.resources.sounds.length === 0 &&
      this.resources.music.length === 0) {
  ...
  startReady() {
    ...
    if (graphics === "M3D") {
      global.M3D = M3D;
      M3D.runtime = this;
    } else if (graphics === "M2D") {
      global.M2D = M2D;
      M2D.runtime = this;
    } else if (graphics.toLowerCase().startsWith("pixi")) {
      global.PIXI = PIXI;
      PIXI.runtime = this;
    } else if (graphics.toLowerCase().startsWith("babylon")) {
    ...
    this.vm.listener = this.listener;
    this.vm.context.global.print = this.vm.context.meta.print;
    if (window.ms_use_server) {
      console.warn('[Runtime] Multiplayer is not supported in microRunner. ServerConnection is not available.');
    }
  ...
  timer() {
    var ds, dt, i, j, ref, time, update_rate;
    if (this.stopped) {
      return;
    }
    requestAnimationFrame(() => {
    ...
    this.last_time = time;
    this.vm.context.global.system.fps = Math.round(1000 / this.dt);
    ...
    if ((ds === 0 || ds === 2) && update_rate === 60 && Math.abs(1000 / this.dt - 60) < 2) {
      ds = 1;
  ...
  reportWarnings() {
    ...
    if (this.vm != null) {
      ...
      for (key in ref) {
        ...
        if (!value.reported) {
          value.reported = true;
          this.listener.reportWarning({
            message: 'Called something that is not a function: "' + value.expression + '"',
            type: "non_function",
            line: value.line,
            column: value.column,
            file: value.file
      ...
      for (key in ref1) {
        ...
        if (!value.reported) {
          value.reported = true;
          this.listener.reportWarning({
            message: 'Used uninitialized variable: "' + value.expression + '"',
      ...
      for (key in ref2) {
        ...
        if (!value.reported) {
          value.reported = true;
          this.listener.reportWarning({
            message: 'Assigned property to uninitialized variable: "' + value.expression + '"',
      ...
      for (key in ref3) {
        ...
        if (!value.reported) {
          value.reported = true;
          this.listener.reportWarning({
            message: 'Overwrote system variable: "' + value.expression + '"',
