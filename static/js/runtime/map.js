    ...
    req.onreadystatechange = (event) => {
      if (req.readyState === XMLHttpRequest.DONE) {
        if (req.status === 200) {
          ...
          console.error('[Map] HTTP error:', req.status, url);
          if (this.runtime?.listener?.log) {
            this.runtime.listener.log('ERROR: Failed to load map file - ' + url + ' (HTTP ' + req.status + ')');
          }
        }
      }
    };
    req.onerror = () => {
      console.error('[Map] Network error:', url);
      if (this.runtime?.listener?.log) {
        this.runtime.listener.log('ERROR: Failed to load map file - ' + url);
      }
    };
    req.open("GET", url);
    return req.send();
...
this.LoadMap = function(url, loaded) {
  ...
  req.onreadystatechange = (event) => {
    if (req.readyState === XMLHttpRequest.DONE) {
      ...
      if (req.status === 200) {
        ...
        console.error('[Map] HTTP error:', req.status, url);
        if (typeof window !== 'undefined' && window.player?.runtime?.listener?.log) {
          window.player.runtime.listener.log('ERROR: Failed to load map - ' + url + ' (HTTP ' + req.status + ')');
        }
      }
      map.needs_update = true;
      if (loaded != null) {
  ...
  req.onerror = () => {
    console.error('[Map] Network error:', url);
    if (typeof window !== 'undefined' && window.player?.runtime?.listener?.log) {
      window.player.runtime.listener.log('ERROR: Failed to load map - ' + url);
    }
  };
  req.open("GET", url);
  req.send();
